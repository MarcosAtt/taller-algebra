...
evaluar [] _ = 0
evaluar (p:ps) x = (p * (x^n)) + evaluar (limpiar ps) x
    where n = grado (p:ps)
...
multiplicarR :: Racional -> Racional -> Racional
multiplicarR (a, b) (c, d) = armaR (a*c) (b*d)

potenciaR :: Racional -> Int -> Racional
potenciaR _ 0 = (1, 1)
potenciaR r n = multiplicarR (potenciaR r (n-1)) r
...
evaluarZ [] _ = (0, 1)
evaluarZ p x = sumaR (multiplicarR (head p, 1) (potenciaR x n)) (evaluarZ (limpiarZ (tail p)) x)
        where n = gradoZ p
